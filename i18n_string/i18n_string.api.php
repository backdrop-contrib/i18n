 ...
 *
 * Basically we are collecting translatable strings for each text group.
 * There are two ways a module can produce this list of strings.
 * It should be one or the other, not both.
 *
 * 1. Provide a list of objects that are translatable for that text group
 *    either defining a 'list callback' for that object type
 *    or implementing hook_i18n_string_objects($type) for that object type.
 *
 * 2. Provide a full list of strings for that text group by implementing
 *    hook_i18n_string_list()
 *
 * Then we have hook_i18n_string_list_TEXTGROUP_alter() for other modules
 * to alter either the list of strings for a single object
 * or the full list of strings at the end.
 */
 ...
 *
 * A module can provide either a list of translatable strings
 * using hook_i18n_string_list() or it can provide a list of objects
 * using hook_i18n_string_objects() from which the string list
 * will be produced automatically. But not both.
 *
 * @param string $group
 *   Text group name.
 */
 ...
 *
 * To build a list of translatable strings for a text group,
 * we'll follow these steps:
 * 1. Invoke hook_i18n_string_list($textgroup), that will get us
 *    an array of strings
 ...
 * 5. Run this hook to alter the strings for that specific object.
 *    In this case we'll pass the $type and $object parameters.
 * 6. Merge all strings from all objects in an array indexed by
 *    textgroup, type, id, name
 * 7. Run this hook once again to alter *all* strings for this textgroup.
 *    In this case we don't have a $type and $object parameters.
 *
 * Thus this hook is really invoked once per object
 * and once per textgroup on top of that.
 *
 ...
 *   we are altering the full list of strings for the text group
 *   that happens once at the end.
 */
 ...
 * A module can provide either a list of translatable strings
 * using hook_i18n_string_list() or it can provide a list of objects
 * using hook_i18n_string_objects() from which the string list will be
 * produced automatically. But not both.
 *
